...
final class USBHelperClient: NSObject, ObservableObject {
    ...
    /// The lifetime rule: no attached devices, no connection. Called
    /// after every one-off call and after every state event.
    ...
    func fetchHelperVersion() async -> Result<String, USBHelperError> {
        ...
        let result: Result<String, USBHelperError> = await withCheckedContinuation { cont in
            let proxy = c.remoteObjectProxyWithErrorHandler { error in
                cont.resume(returning: .failure(.xpc(error.localizedDescription)))
    ...
    func handleStateChanged(_ device: AVMUSBDeviceIdentity,
        ...
        case .failed:
            attachedDevices.remove(device)
            let step = detail ?? "unknown step"
            announce("Could not attach \(name). Step: \(step). Next: check that no other app is using the device and try again.", tone: .failure)
        case .yanked:
            attachedDevices.remove(device)
